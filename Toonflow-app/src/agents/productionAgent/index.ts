...
import { getDirectorSkillSections } from "@/utils/skillManual";
...
export async function runDecisionAI(ctx: AgentContext) {
  const { isolationKey, text, abortSignal, userId } = ctx;
  const memory = new Memory("productionAgent", isolationKey, userId, ctx.textModel);
  ...
  const { fullStream } = await u.Ai.Text("productionAgent:decisionAgent", ctx.userId, ctx.thinkConfig.think, ctx.thinkConfig.thinlLevel, ctx.textModel).stream({
    messages: [
...
async function createSubAgent(parentCtx: AgentContext) {
  const { resTool, abortSignal, userId } = parentCtx;
  const memory = new Memory("productionAgent", parentCtx.isolationKey, userId, parentCtx.textModel);
  async function runAgent({
    ...
    const { fullStream } = await u.Ai.Text(key, userId, parentCtx.thinkConfig.think, parentCtx.thinkConfig.thinlLevel, parentCtx.textModel).stream({
      system,
...
async function createArtSkills(artName: string, storyName: string) {
  const [artSections, storySections] = await Promise.all([getDirectorSkillSections("art", artName), getDirectorSkillSections("story", storyName)]);
  const mainSkills: { path: string; name: string; description: string; content: string }[] = [];
  for (const { sectionKey, content } of [...artSections, ...storySections]) {
    const parsed = parseFrontmatter(content);
    mainSkills.push({ path: `skill_manual://${sectionKey}`, content, ...parsed });
  }
  const res = {
    prompt: `## Skills
以下技能提供了专业任务的专用指令。
...
async function useProductionSkills(artName: string, storyName: string) {
  ...
  const [artSections, storySections, productionSkillList] = await Promise.all([
    getDirectorSkillSections("art", artName),
    getDirectorSkillSections("story", storyName),
    scanSkills(productionPath + "/*.md"),
  ]);
  const mainSkills: { path: string; name: string; description: string; content?: string }[] = [];
  for (const { sectionKey, content } of [...artSections, ...storySections]) {
    const parsed = parseFrontmatter(content);
  ...
  for (const skillPath of productionSkillList) {
    if (!fs.existsSync(skillPath)) throw new Error(`主技能文件不存在: ${skillPath}`);
    const content = await fs.promises.readFile(skillPath, "utf-8");
    const parsed = parseFrontmatter(content);
    mainSkills.push({ path: skillPath, ...parsed });
