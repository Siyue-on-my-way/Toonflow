...
export async function runQuickVideoAgent(ctx: AgentContext) {
  ...
  const projectInfo = [
    ...
    ctx.mode === "image"
      ...
      : ctx.mode === "video"
        ? `本轮用户在聊天框选择了「视频」生成模式，模型：${ctx.videoModel}。用户可以直接通过纯文本描述生成短视频，请顺畅调用 generate_video 工具按用户描述生成视频（无需强制要求用户提供参考图；无图时系统将自适应文生视频或自动生成概念首帧图链式生视频）。生成的视频和概念图将自动进入聊天记录和资产白板，不会自动绑定到任何镜头或自动确认分镜。`
        : "",
    "",
    mem,
  ]
