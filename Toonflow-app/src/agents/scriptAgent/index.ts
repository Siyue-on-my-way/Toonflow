...
export async function runDecisionAI(ctx: AgentContext) {
  ...
  const memory = new Memory("scriptAgent", isolationKey, userId, ctx.textModel);
  ...
  const { fullStream } = await u.Ai.Text("scriptAgent:decisionAgent", ctx.userId, ctx.thinkConfig.think, ctx.thinkConfig.thinlLevel, ctx.textModel).stream({
    messages: [
...
function createSubAgent(parentCtx: AgentContext) {
  ...
  const memory = new Memory("scriptAgent", parentCtx.isolationKey, userId, parentCtx.textModel);
