...
export default async (knex: Knex): Promise<void> => {
  ...
  await addColumn("o_tasks", "leaseUntil", "bigInteger");
  await addColumn("o_tasks", "recoveryAttempts", "integer");
  if (!(await knex.schema.hasTable("o_generation_tasks"))) {
    await knex.schema.createTable("o_generation_tasks", (table) => {
      table.string("taskId").notNullable().primary();
      ...
      table.text("reason");
      table.text("resultPath");
      table.bigInteger("createdAt");
      table.bigInteger("updatedAt");
      table.bigInteger("lastPolledAt");
      table.bigInteger("leaseUntil");
      table.integer("retryCount").defaultTo(0);
      table.index(["provider", "providerTaskId"]);
      table.index(["storyboardId", "status"]);
    });
  }
  ...
  await addColumn("o_assets", "sourceType", "text");
  await addColumn("o_assets", "sourceAssetId", "integer");
  await addColumn("o_modelPrompt", "fileName", "string");
  await addColumn("o_modelPrompt", "path", "string");
  const vendorDataSelect = await u.db("o_vendorConfig").whereIn("id", ["aibotplatform", "deepseek", "atlascloud", "runninghub"]).select("*");
  ...
  if (!vendorDataSelect.find((i) => i.id == "runninghub")) {
    await u.db("o_vendorConfig").insert({
      id: "runninghub",
      inputValues: "{}",
      models: "[]",
  ...
  // AibotPlatform 接入字段和 gpt-5.6-terra 协议目录迁移。
  // initDB 只对新建表执行，已有部署必须在这里幂等补齐；routeID/token 等运行时值不在代码中臆造。
  const aibotplatformConfig = vendorDataSelect.find((item) => item.id === "aibotplatform");
  if (!aibotplatformConfig) {
    await db("o_vendorConfig").insert({
      id: "aibotplatform",
      enable: 1,
      inputValues: JSON.stringify(aibotplatformVendor.inputValues),
      models: JSON.stringify(aibotplatformVendor.models),
    });
  } else {
    let inputValues: Record<string, any> = {};
    try {
      inputValues = JSON.parse(aibotplatformConfig.inputValues || "{}");
    } catch {}

    const migratedInputValues: Record<string, any> = {
      ...inputValues,
      appID: inputValues.appID ?? "",
      businessSource: inputValues.businessSource ?? "",
      token: inputValues.token || inputValues.apiKey || "",
      modelRoutes: inputValues.modelRoutes ?? "",
      baseUrl: inputValues.baseUrl || aibotplatformVendor.inputValues.baseUrl,
    };
    delete migratedInputValues.apiKey;

    let models: any[] = [];
    try {
      const parsedModels = JSON.parse(aibotplatformConfig.models || "[]");
      if (Array.isArray(parsedModels)) models = parsedModels;
    } catch {}

    let modelsChanged = false;
    for (const model of aibotplatformVendor.models) {
      if (!models.some((existing) => existing?.modelName === model.modelName)) {
        models.push(model);
        modelsChanged = true;
      }
    }

    const inputValuesChanged = JSON.stringify(inputValues) !== JSON.stringify(migratedInputValues);
    if (inputValuesChanged || modelsChanged) {
      await db("o_vendorConfig")
        .where("id", "aibotplatform")
        .update({
          ...(inputValuesChanged ? { inputValues: JSON.stringify(migratedInputValues) } : {}),
          ...(modelsChanged ? { models: JSON.stringify(models) } : {}),
        });
    }
  }

  // Kling O1 是本次 RunningHub 协议目录新增项；同步缺口可让已有部署直接在设置页启用。
  // 重新读取一行，覆盖“首次安装时刚插入渠道”的情况，也避免使用上一步查询的旧 models 快照。
  const runninghubConfig = await u.db("o_vendorConfig").where("id", "runninghub").first();
  if (runninghubConfig) {
    let catalogModels: any[] = [];
    try {
      const parsed = JSON.parse(runninghubConfig.models || "[]");
      if (Array.isArray(parsed)) catalogModels = parsed;
    } catch {}
    if (!catalogModels.some((model: { modelName?: string }) => model?.modelName === KLING_O1_VENDOR_MODEL.modelName)) {
      catalogModels.push(KLING_O1_VENDOR_MODEL);
      await u.db("o_vendorConfig").where("id", "runninghub").update({ models: JSON.stringify(catalogModels) });
    }
  }

  // GPT Image (rhart-image-g-2) 协议目录迁移：幂等补齐目录并启用，确保设置页和生产入口可见。
  // enabledModels 可能是旧部署尚未拥有的列，必须先幂等补列再执行 upsert。
  await addColumn("o_vendorConfig", "enabledModels", "text");
  const runninghubCatalogRow = await u.db("o_vendorConfig").where("id", "runninghub").first();
  if (runninghubCatalogRow) {
    let catalogModels: any[] = [];
    try {
      const parsed = JSON.parse(runninghubCatalogRow.models || "[]");
      if (Array.isArray(parsed)) catalogModels = parsed;
    } catch {}
    const gptImageModel = runninghubVendor.models.find((model) => model.modelName === "rhart-image-g-2");
    if (gptImageModel) {
      const index = catalogModels.findIndex((model: any) => model?.modelName === gptImageModel.modelName);
      if (index < 0) catalogModels.push(gptImageModel);
      else catalogModels[index] = gptImageModel;
      let enabledModels: string[] = [];
      try {
        const parsed = JSON.parse(runninghubCatalogRow.enabledModels || "[]");
        if (Array.isArray(parsed)) enabledModels = parsed.filter((name): name is string => typeof name === "string");
      } catch {}
      if (!enabledModels.includes(gptImageModel.modelName)) enabledModels.push(gptImageModel.modelName);
      await u.db("o_vendorConfig").where("id", "runninghub").update({
  ...
  // SIY-67/SIY-105：Seedance 2.0 Fast / Mini 协议目录迁移。设置页从 o_vendorConfig.models 读取，
  // 因此不能只依赖 runninghub.ts 静态声明；老部署启动时幂等补入数据库目录并启用。
  for (const seedanceModelName of ["seedance-2.0-fast", "seedance-2.0-mini"]) {
    const seedanceModel = runninghubVendor.models.find((model) => model.modelName === seedanceModelName);
    if (!seedanceModel) continue;
    const seedanceRow = await u.db("o_vendorConfig").where("id", "runninghub").first();
    if (seedanceRow) {
      let seedanceCatalog: any[] = [];
      try {
        const parsed = JSON.parse(seedanceRow.models || "[]");
        if (Array.isArray(parsed)) seedanceCatalog = parsed;
      } catch {}
      const existingIndex = seedanceCatalog.findIndex((model: any) => model?.modelName === seedanceModel.modelName);
      if (existingIndex < 0) seedanceCatalog.push(seedanceModel);
      else seedanceCatalog[existingIndex] = { ...seedanceCatalog[existingIndex], ...seedanceModel };

      let enabledModels: string[] = [];
      try {
        const parsed = JSON.parse(seedanceRow.enabledModels || "[]");
        if (Array.isArray(parsed)) enabledModels = parsed.filter((name): name is string => typeof name === "string");
      } catch {}
      if (!enabledModels.includes(seedanceModel.modelName)) enabledModels.push(seedanceModel.modelName);

      await u.db("o_vendorConfig").where("id", "runninghub").update({
        models: JSON.stringify(seedanceCatalog),
        enabledModels: JSON.stringify(enabledModels),
      });
    }
  }

  // Nano Banana Pro 协议目录迁移：已有部署启动时补齐。目录能力与启用状态分离，初次补入后自动加入 enabledModels，保证基础目标“设置页可见、生产入口可用”。
  const nanoBananaModel = NANO_BANANA_PRO_VENDOR_MODEL;
  const nanoBananaRow = await u.db("o_vendorConfig").where("id", "runninghub").first();
  if (nanoBananaRow) {
    let nanoCatalog: any[] = [];
    try {
      const parsed = JSON.parse(nanoBananaRow.models || "[]");
      if (Array.isArray(parsed)) nanoCatalog = parsed;
    } catch {}
    const existingIndex = nanoCatalog.findIndex((model: any) => model?.modelName === nanoBananaModel.modelName);
    if (existingIndex < 0) nanoCatalog.push(nanoBananaModel);
    else nanoCatalog[existingIndex] = { ...nanoCatalog[existingIndex], ...nanoBananaModel };

    let enabledModels: string[] = [];
    try {
      const parsed = JSON.parse(nanoBananaRow.enabledModels || "[]");
      if (Array.isArray(parsed)) enabledModels = parsed.filter((name): name is string => typeof name === "string");
    } catch {}
    if (!enabledModels.includes(nanoBananaModel.modelName)) enabledModels.push(nanoBananaModel.modelName);
    await u.db("o_vendorConfig").where("id", "runninghub").update({
      models: JSON.stringify(nanoCatalog),
      enabledModels: JSON.stringify(enabledModels),
    });
  }

  // SIY-64：Nano Banana 2 协议目录迁移。设置页从 o_vendorConfig.models 读取，
  // 因此已有部署必须幂等补入数据库目录；同时加入 enabledModels，保证设置页可见且生产入口可选。
  const nanoBanana2Model = runninghubVendor.models.find((model) => model.modelName === "rhart-image-n-g31-flash");
  const nanoBanana2Row = await u.db("o_vendorConfig").where("id", "runninghub").first();
  if (nanoBanana2Model && nanoBanana2Row) {
    let nanoBanana2Catalog: any[] = [];
    try {
      const parsed = JSON.parse(nanoBanana2Row.models || "[]");
      if (Array.isArray(parsed)) nanoBanana2Catalog = parsed;
    } catch {}
    const existingIndex = nanoBanana2Catalog.findIndex((model: any) => model?.modelName === nanoBanana2Model.modelName);
    if (existingIndex < 0) nanoBanana2Catalog.push(nanoBanana2Model);
    else nanoBanana2Catalog[existingIndex] = { ...nanoBanana2Catalog[existingIndex], ...nanoBanana2Model };

    let enabledModels: string[] = [];
    try {
      const parsed = JSON.parse(nanoBanana2Row.enabledModels || "[]");
      if (Array.isArray(parsed)) enabledModels = parsed.filter((name): name is string => typeof name === "string");
    } catch {}
    if (!enabledModels.includes(nanoBanana2Model.modelName)) enabledModels.push(nanoBanana2Model.modelName);
    await u.db("o_vendorConfig").where("id", "runninghub").update({
      models: JSON.stringify(nanoBanana2Catalog),
      enabledModels: JSON.stringify(enabledModels),
    });
  }

  await addColumn("o_user", "role", "string");
  await db("o_user").where("id", 1).whereNull("role").update({ role: "admin" });
  await db("o_user").whereNot("id", 1).whereNull("role").update({ role: "member" });

  // 多账号数据层：把现有 o_vendorConfig 的 key 和已启用模型迁移到 admin(userId=1) 名下的 o_vendorAccount 记录，
  // 保证升级后 admin 账号能无缝继续使用现有配置。
  // 注意：o_vendorConfig.inputValues 暂不清空密钥——当前所有渠道调用（src/utils/ai.ts getVendorTemplateFn）仍直接从
  // o_vendorConfig 读取并解密密钥，清空会立即打断现有模型调用；待"渠道接口账号化改造"子任务把调用逻辑切到
  // o_vendorAccount 之后，再收敛/清空 o_vendorConfig 里的密钥类字段。
  const vendorConfigsForAccount = await db("o_vendorConfig").select("*");
  for (const vc of vendorConfigsForAccount) {
    const existingAccount = await db("o_vendorAccount").where({ userId: 1, vendorId: vc.id }).first();
    if (existingAccount) continue;

    let inputValues: Record<string, any> = {};
    try {
      inputValues = JSON.parse(vc.inputValues || "{}");
    } catch {}
    let models: any[] = [];
    try {
      models = JSON.parse(vc.models || "[]");
    } catch {}
    const enabledModels = models.map((m) => m?.modelName).filter(Boolean);

    await db("o_vendorAccount").insert({
      userId: 1,
      vendorId: vc.id,
      enable: vc.enable ?? 0,
  ...
  // 渠道 Key / 模型启用收回为全局配置（仅 admin 可写，参见 SIY-65）：o_vendorConfig 新增 enabledModels 列，
  // 把之前"账号化改造"阶段迁到 admin(role=admin，通常是 userId=1) 名下 o_vendorAccount 的 Key/已启用模型，
  // 一次性搬回 o_vendorConfig，此后所有渠道调用/设置页读写都只认 o_vendorConfig 这一份全局数据，
  // 不再区分是哪个账号在操作。仅在 o_vendorConfig.enabledModels 还是空值时回填，保证幂等、不覆盖后续已产生的新数据。
  await addColumn("o_vendorConfig", "enabledModels", "text");
  const adminUser = await db("o_user").where("role", "admin").orderBy("id", "asc").first();
  if (adminUser) {
    const vendorConfigsForGlobalize = await db("o_vendorConfig").select("*");
    for (const vc of vendorConfigsForGlobalize) {
      if (vc.enabledModels != null) continue; // 已回填过，跳过

      const adminAccount = await db("o_vendorAccount").where({ userId: adminUser.id, vendorId: vc.id }).first();
      if (!adminAccount) {
        await db("o_vendorConfig").where("id", vc.id).update({ enabledModels: "[]" });
        continue;
      }

      const patch: Record<string, any> = { enabledModels: adminAccount.enabledModels ?? "[]" };
      const hasOwnKey = vc.inputValues && vc.inputValues !== "{}";
      if (!hasOwnKey && adminAccount.inputValues && adminAccount.inputValues !== "{}") {
        patch.inputValues = adminAccount.inputValues;
      }
      if (!vc.enable && adminAccount.enable) {
        patch.enable = adminAccount.enable;
      }
      await db("o_vendorConfig").where("id", vc.id).update(patch);
    }
  } else {
    await db("o_vendorConfig").whereNull("enabledModels").update({ enabledModels: "[]" });
  }

  // SIY-65：把早期手工录入的 RunningHub minimax-h3-oss 协议补齐为官方 API
  // 声明的完整能力。只升级已知的旧三模式配置，避免覆盖管理员后来维护过的协议。
  const runningHubConfig = await db("o_vendorConfig").where("id", "runninghub").first();
  if (runningHubConfig?.models) {
    let runningHubModels: any[] = [];
    try {
      const parsedModels = JSON.parse(runningHubConfig.models);
      if (Array.isArray(parsedModels)) runningHubModels = parsedModels;
    } catch {}

    const legacyModes = ["text", "startFrameOptional", "endFrameOptional"];
    const fullModes = ["singleImage", "startEndRequired", "endFrameOptional", "startFrameOptional", "text"];
    ...
    for (const model of runningHubModels) {
      if (model?.modelName !== "rhart-video/minimax-h3-oss/fl2va" || model.type !== "video") continue;

      const currentModes = Array.isArray(model.mode) ? model.mode : [];
      const isLegacyModeSet =
        currentModes.length === legacyModes.length && legacyModes.every((mode) => currentModes.includes(mode));
      if (isLegacyModeSet) {
        model.mode = fullModes;
        changed = true;
      }
      if (!Array.isArray(model.aspectRatioOptions) || model.aspectRatioOptions.length === 0) {
        model.aspectRatioOptions = aspectRatioOptions;
        changed = true;
      }
    }

    if (changed) {
      await db("o_vendorConfig").where("id", "runninghub").update({ models: JSON.stringify(runningHubModels) });
    }
  }
  ...
  // 工作流图片等任务会保存完整请求 JSON，MySQL 的 varchar(255) 不足以容纳它。
  // 将旧版本创建的 varchar 列升级为 text；新建数据库由 initDB 直接创建为 text。
