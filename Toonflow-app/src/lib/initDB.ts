...
export default async (knex: Knex, forceInit: boolean = false): Promise<void> => {
  const tables: TableSchema[] = [
      ...
      initData: async (knex) => {
        await knex("o_agentDeploy").insert([
          ...
          },
          {
            ...
            key: "quickVideoAgent",
            name: "快创Agent",
            desc: "单视频快创模式的创作Agent，负责简报梳理与分镜生成，建议使用具备较强文本理解和结构化输出能力的模型",
            disabled: false,
          },
          {
            model: "",
    ...
    // 持久化的分镜生图任务：taskId 对外可查询，providerTaskId 用于重启恢复。
    {
      name: "o_generation_tasks",
      builder: (table) => {
    ...
    //任务中心表
    {
      ...
      builder: (table) => {
        ...
        table.string("providerTaskId");
        table.bigInteger("leaseUntil");
        table.integer("recoveryAttempts").defaultTo(0);
        table.primary(["id"]);
        table.unique(["id"]);
      },
      initData: async (knex) => {},
    ...
    //资产表
    {
      ...
      builder: (table) => {
        ...
        table.text("sourceType"); //资产来源，例如 library
        table.integer("sourceAssetId"); //来源 o_library_asset.id
        table.primary(["id"]);
        table.unique(["id"]);
      },
      initData: async (knex) => {},
    ...
    //flowData-剧本
    {
      ...
      builder: (table) => {
        ...
        table.text("data", "longtext"); // planData/flowData JSON 可能很大，varchar(255) 会 ER_DATA_TOO_LONG
        table.bigInteger("createTime");
    ...
    //视频
    {
      ...
      builder: (table) => {
        ...
        table.text("filePath");
        table.text("errorReason");
        table.bigInteger("time"); // 存毫秒时间戳 Date.now()，integer 会溢出
        table.text("state");
        table.integer("scriptId");
        table.integer("projectId");
    ...
    //供应商配置表
    {
      ...
      builder: (table) => {
        ...
        table.text("enabledModels"); // 全局启用的模型名列表 JSON，admin 专属维护，所有账号共享同一份
        table.primary(["id"]);
        table.unique(["id"]);
      },
      initData: async (knex) => {
    ...
    //供应商账号配置表（每个用户在每个渠道下的私有 key/token 与已启用模型）
    {
      name: "o_vendorAccount",
      builder: (table) => {
        table.increments("id").primary();
        table.integer("userId").notNullable();
        table.string("vendorId").notNullable();
        table.integer("enable").defaultTo(0); // 该账号是否启用该渠道
        table.text("inputValues"); // 该账号自己的输入项值（apiKey/token 等）JSON
        table.text("enabledModels"); // 该账号从渠道模型目录里选用的 modelName 列表 JSON string[]
    ...
    },
    {
      ...
      builder: (table) => {
        table.string("skillId", 64).notNullable().references("id").inTable("o_skillList").onDelete("CASCADE");
        table.string("attribution", 255).notNullable(); // "production_agent_decision.md" | "production_agent_execution.md" | "production_agent_supervision.md" | "script_agent_decision.md" | "script_agent_execution.md" | "script_agent_supervision.md" | "universal_agent.md"
    ...
    //记忆表（message=原始消息, summary=压缩摘要）
    {
      ...
      builder: (table) => {
        table.string("id", 64).notNullable();
        table.string("isolationKey", 255).notNullable(); // 记忆隔离键
    ...
    // 视觉手册(art_skills)/导演手册(story_skills) 文本内容，替代直接读写 data/skills 下的 md 文件
    {
      name: "skill_manual",
      builder: (table) => {
        table.string("id", 64).notNullable();
        table.string("type", 16).notNullable(); // 'art' | 'story'
    ...
    // 视觉手册/导演手册关联的图片在 MinIO 中的 key，用于新增/编辑时的保留-删除对比与整体删除
    {
      name: "skill_manual_image",
      builder: (table) => {
        table.string("id", 64).notNullable();
