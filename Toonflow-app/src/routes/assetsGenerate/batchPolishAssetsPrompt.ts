...
export default router.post(
  ...
  async (req, res) => {
    const { projectId, items, concurrentCount, otherTextPrompt, textModel } = req.body;
    ...
    const tasks = items.map((item: { assetsId: number; type: string; name: string; describe: string }) =>
      limit(async () => {
        ...
        try {
          const { _output } = (await u.Ai.Text("universalAi", (req as any).user.id, undefined, 0, modelOverride as `${string}:${string}` | undefined).invoke({
            system: systemPrompt + (otherTextPrompt ? "\n" + otherTextPrompt : ""),
            messages: [
              {
                role: "user",
                content: `
