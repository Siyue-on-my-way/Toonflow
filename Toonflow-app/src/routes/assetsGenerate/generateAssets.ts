...
const requestSchema = {
  ...
  // 多张参考图（<=10 张，与单张 base64 兼容并存）
  base64List: z.array(z.string()).optional().nullable(),
  aspectRatio: z.string().optional(),
};

export default router.post("/", validateFields(requestSchema), async (req, res) => {
  const { projectId, model, resolution, id, type, name, prompt, base64, base64List, aspectRatio } = req.body;
  ...
  try {
    const aiImage = u.Ai.Image(model, (req as any).user.id);
    await aiImage.run(
      {
        prompt: userPrompt,
        referenceList: [
          ...(Array.isArray(base64List) ? base64List.filter(Boolean).map((item) => ({ type: "image" as const, base64: item })) : []),
          ...(base64 ? [{ type: "image" as const, base64 }] : []),
        ],
        size: resolution,
