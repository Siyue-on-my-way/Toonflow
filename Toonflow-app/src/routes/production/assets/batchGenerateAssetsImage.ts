...
import { getProjectTextModel } from "@/utils/projectModel";
...
export default router.post(
  ...
  async (req, res) => {
    const { assetIds, projectId, scriptId, concurrentCount = 5, textModel, imageModel } = req.body;
    const modelOverride = await getProjectTextModel(projectId, textModel);
    ...
    const rolePrompt = await u.getArtPrompt(projectSettingData!.artStyle!, "art_skills", "art_character_derivative");
    const toolPrompt = await u.getArtPrompt(projectSettingData!.artStyle!, "art_skills", "art_prop_derivative");
    const scenePrompt = await u.getArtPrompt(projectSettingData!.artStyle!, "art_skills", "art_scene_derivative");
    ...
    const generateSingleAsset = async (item: any) => {
      ...
      const { text } = await u.Ai.Text("universalAi", (req as any).user.id, undefined, 0, modelOverride as `${string}:${string}` | undefined).invoke({
