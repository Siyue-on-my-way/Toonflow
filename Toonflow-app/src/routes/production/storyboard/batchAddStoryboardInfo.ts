...
export default router.post(
  ...
  async (req, res) => {
    ...
    let nextStoryboardId = Number(storyboardMaxRow?.maxId ?? 0) + 1;
    for (const item of data) {
      const id = nextStoryboardId++;
    ...
    let nextTrackId = Number(trackMaxRow?.maxId ?? 0) + 1;
    ...
    // trackId 更新后重新查询，保证返回数据携带最新 trackId（修复 trackId 回显为 null）
    const updatedStoryboard = await u.db("o_storyboard").where("scriptId", scriptId);
    const storyboardData = await Promise.all(
      updatedStoryboard.map(async (i) => {
        return {
          associateAssetsIds: await orderByInsertOrder(u.db("o_assets2Storyboard").where("storyboardId", i.id)).select("assetId").pluck("assetId"),
