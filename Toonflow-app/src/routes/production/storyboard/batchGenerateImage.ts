import express from "express";
import u from "@/utils";
import { orderByInsertOrder } from "@/utils/db";
import { z } from "zod";
import sharp from "sharp";
import { error, success } from "@/lib/responseFormat";
import { validateFields } from "@/middleware/middleware";
import { Output, tool } from "ai";
import { v4 as uuid } from "uuid";
import { assetItemSchema } from "@/agents/productionAgent/tools";
const router = express.Router();
export type AssetData = z.infer<typeof assetItemSchema>;

export default router.post(
  "/",
  validateFields({
    storyboardIds: z.array(z.number()),
    projectId: z.number(),
    scriptId: z.number(),
    concurrentCount: z.number().min(1).optional(),
    compulsory: z.boolean().optional(),
    imageModel: z.string().optional(),
  }),
  async (req, res) => {
    const {
      storyboardIds,
      projectId,
      scriptId,
      concurrentCount = 5,
      compulsory = false,
      imageModel,
    }: {
      storyboardIds: number[];
      projectId: number;
      scriptId: number;
      concurrentCount: number;
      compulsory: boolean;
      imageModel?: string;
    } = req.body;
    if (!storyboardIds || storyboardIds.length === 0) return res.status(400).send(error("storyboardIds不能为空"));
    // 当没有 storyboardIds 时，通过 AI 生成新的分镜面板数据
    let finalStoryboardIds: number[] = storyboardIds || [];
    // shouldGenerateImage === 0 的分镜标记为「未生成」，其余标记为「生成中」
    const storyboardData = await u.db("o_storyboard").where("scriptId", scriptId).where("projectId", projectId).whereIn("id", finalStoryboardIds);
    if (!storyboardData.length) return res.status(500).send(error("未查到分镜数据"));
    const storyIds = storyboardData.map((i) => i.id);

    const projectSettingData = await u.db("o_project").where("id", projectId).select("imageModel", "imageQuality", "artStyle", "videoRatio").first();
    const selectedImageModel = imageModel || projectSettingData?.imageModel;
    if (!selectedImageModel) return res.status(400).send(error("未配置默认图片模型，请先修复项目配置"));
    const [selectedVendorId, selectedModelName] = selectedImageModel.split(/:(.+)/);
    if (!selectedVendorId || !selectedModelName) return res.status(400).send(error("图片模型配置格式无效，请先修复项目配置"));
    const selectedVendor = u.vendor.getVendorModule(selectedVendorId);
    if (!selectedVendor) return res.status(400).send(error(`图片模型供应商不存在: ${selectedVendorId}`));
    const selectedModels = await u.vendor.getModelList(selectedVendorId);
    const selectedModel = selectedModels.find((model: any) => model.modelName === selectedModelName);
    const enabledModels = await u.vendor.getEnabledModelNames(selectedVendorId);
    if (!selectedModel || selectedModel.type !== "image") return res.status(400).send(error(`图片模型不存在或不是图片模型: ${selectedImageModel}`));
    if (!enabledModels.includes(selectedModelName)) return res.status(400).send(error(`图片模型未启用，请先在设置中启用: ${selectedImageModel}`));

    // shouldGenerateImage === 0 的分镜标记为「未生成」，其余标记为「生成中」
    if (compulsory) {
      await u.db("o_storyboard").whereIn("id", storyIds).where("scriptId", scriptId).update({ state: "生成中", shouldGenerateImage: 1 });
    } else {
      await u.db("o_storyboard").whereIn("id", storyIds).where("scriptId", scriptId).where("shouldGenerateImage", 0).update({ state: "未生成" });
      await u.db("o_storyboard").whereIn("id", storyIds).where("scriptId", scriptId).where("shouldGenerateImage", 1).update({ state: "生成中" });
    }

    // 按插入顺序查出每个 storyboard 关联的 assetId 有序列表
    const assets2StoryboardRows = await orderByInsertOrder(
      u.db("o_assets2Storyboard").whereIn("storyboardId", storyIds),
    ).select("storyboardId", "assetId");

    // 收集所有 assetId，批量查对应的 imageId
    const allAssetIds = [...new Set(assets2StoryboardRows.map((r: any) => r.assetId))];
    const assetImageMap: Record<number, number> = {};
    if (allAssetIds.length > 0) {
      const assetRows = await u.db("o_assets").whereIn("id", allAssetIds).select("id", "imageId");
      assetRows.forEach((row: any) => {
        assetImageMap[row.id] = row.imageId;
      });
    }

    // 按关联记录的显式顺序重建 assetRecord，值为有序的 imageId 列表
    const assetRecord: Record<number, number[]> = {};
    assets2StoryboardRows.forEach((item: any) => {
      if (!assetRecord[item.storyboardId]) {
        assetRecord[item.storyboardId] = [];
      }
      const imageId = assetImageMap[item.assetId];
      if (imageId != null) {
        assetRecord[item.storyboardId].push(imageId);
      }
    });
    const generationTasks = storyboardData
      .filter((item) => compulsory || item.shouldGenerateImage !== 0)
      .map((item) => ({
        taskId: uuid(),
        storyboardId: item.id!,
        projectId,
        scriptId,
        provider: selectedVendorId,
        model: selectedModelName,
        status: "CREATED",
        createdAt: Date.now(),
        updatedAt: Date.now(),
        retryCount: 0,
      }));
    await u.db("o_generation_tasks").insert(generationTasks);
    res.status(202).send(success(generationTasks.map(({ taskId, storyboardId, status }) => ({ taskId, storyboardId, status }))));

    const taskByStoryboard = new Map(generationTasks.map((task) => [task.storyboardId, task]));
    const generateTask = async (item: (typeof storyboardData)[number]) => {
      const localTask = taskByStoryboard.get(item.id!);
      if (!localTask) return;
      const repeloadObj = {
        prompt: item.prompt!,
        size: projectSettingData?.imageQuality as "1K" | "2K" | "4K",
        aspectRatio: projectSettingData?.videoRatio as `${number}:${number}`,
      };
      try {
        await u.db("o_generation_tasks").where("taskId", localTask.taskId).update({ status: "SUBMITTED", updatedAt: Date.now() });
        const imageCls = await u.Ai.Image(selectedImageModel as `${string}:${string}`, (req as any).user.id).run(
          {
            referenceList: await getAssetsImageBase64(assetRecord[item.id!] || []),
            ...repeloadObj,
          },
          {
            taskClass: "生成分镜图片",
            describe: "分镜图片生成",
            relatedObjects: JSON.stringify({
              ...repeloadObj,
              imageModel: selectedImageModel,
              storyboardId: item.id,
              scriptId,
              referenceImageIds: assetRecord[item.id!] || [],
            }),
            projectId,
            setProviderTaskId: async (providerTaskId: string) => {
              await u.db("o_generation_tasks").where("taskId", localTask.taskId).update({
                providerTaskId,
                status: "QUEUED",
                updatedAt: Date.now(),
              });
            },
          },
        );
        const savePath = `/${projectId}/assets/${scriptId}/${u.uuid()}.jpg`;
        await imageCls.save(savePath);
        await u.db("o_storyboard").where("id", item.id).update({ filePath: savePath, state: "已完成", reason: null });
        await u.db("o_generation_tasks").where("taskId", localTask.taskId).update({
          status: "SUCCEEDED",
          providerStatus: "SUCCESS",
          resultPath: savePath,
          lastPolledAt: Date.now(),
          updatedAt: Date.now(),
        });
      } catch (e) {
        const reason = u.error(e).message;
        await u.db("o_storyboard").where("id", item.id).update({ filePath: "", reason, state: "生成失败" });
        await u.db("o_generation_tasks").where("taskId", localTask.taskId).update({ status: "FAILED", reason, updatedAt: Date.now() });
      }
    };
    // 按 concurrentCount 控制并发数，分批执行；跳过 shouldGenerateImage === 0 的分镜
    const generateList = compulsory ? storyboardData : storyboardData.filter((item) => item.shouldGenerateImage !== 0);
    for (let i = 0; i < generateList.length; i += concurrentCount) {
      await Promise.all(generateList.slice(i, i + concurrentCount).map(generateTask));
    }

  },
);
async function getAssetsImageBase64(imageIds: number[]) {
  if (!imageIds.length) return [];

  const imagePaths = await u.db("o_image").whereIn("o_image.id", imageIds).select("o_image.id", "o_image.filePath");

  if (imagePaths.length !== imageIds.length) {
    const foundIds = new Set(imagePaths.map((row) => row.id));
    const missingId = imageIds.find((id) => !foundIds.has(id));
    throw new Error(`分镜引用图片记录不存在: imageId=${missingId}`);
  }

  // 建立 id 到 filePath 的映射
  const id2Path = new Map<number, string>();
  for (const row of imagePaths) {
    id2Path.set(row.id, row.filePath);
  }

  // 保证输出顺序与 imageIds 一致
  const imageUrls = await Promise.all(
    imageIds.map(async (id) => {
      const filePath = id2Path.get(id);
      if (!filePath) {
        throw new Error(`分镜引用图片不存在: imageId=${id}`);
      }
      try {
        return await u.oss.getImageBase64(filePath);
      } catch (e) {
        throw new Error(`读取分镜引用图片失败: imageId=${id}, ${u.error(e).message}`);
      }
    }),
  );
  // 保留顺序，并且过滤掉无效项
  return (imageUrls.filter(Boolean) as string[]).map((url) => ({ type: "image" as const, base64: url }));
}
