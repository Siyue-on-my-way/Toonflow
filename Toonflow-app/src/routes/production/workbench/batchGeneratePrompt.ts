...
export default router.post(
  ...
  async (req, res) => {
    const { trackData, projectId, mode, model, concurrentCount = 5, textModel } = req.body;
    ...
    try {
      ...
      const visualManual = await u.getArtPrompt(artStyle, "art_skills", "art_storyboard_video");
      await u
        .db("o_videoTrack")
        .whereIn(
          "id",
            ...
            track.info.map(async (item: { id: number; sources: string }) => {
              if (item.sources === "storyboard") {
                ...
                const storyboard = await u
                  ...
                  .first();
                // 查询分镜关联的资产ID
                const assetRows = await orderByInsertOrder(u.db("o_assets2Storyboard").where("storyboardId", item.id)).select("assetId");
