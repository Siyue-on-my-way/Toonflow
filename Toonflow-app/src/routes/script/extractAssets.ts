...
export default router.post(
  ...
  async (req, res) => {
    const { scriptIds, projectId, groupSize = 5, textModel } = req.body;
      ...
      group.map(async (itemIds) => {
        ...
        try {
          ...
          const existingHint = existingAssetsList
            ...
            : "";
          const output = await u.Ai.Text("universalAi", (req as any).user.id, undefined, 0, modelOverride as `${string}:${string}` | undefined).invoke({
            messages: [
              {
                role: "system",
                content:
