...
export default router.post(
  ...
  async (req, res) => {
    const { content, projectId, textModel } = req.body;
...
4. 如果文本中没有明显的章节分隔模式，返回空字符串。`;
    ...
    const resText = await u.Ai.Text("universalAi", (req as any).user.id, undefined, 0, modelOverride as `${string}:${string}` | undefined).invoke({
      system: systemPrompt,
      messages: [
        {
          role: "user",
