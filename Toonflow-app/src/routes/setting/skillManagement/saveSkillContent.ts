...
import { parseVirtualPath, sectionExists, upsertSection } from "@/utils/skillManual";
...
export default router.post(
  ...
  async (req, res) => {
    ...
    const manual = parseVirtualPath(path);
    if (manual) {
      if (!(await sectionExists(manual.type, manual.styleName, manual.sectionKey))) {
        return res.status(400).send(error("文件不存在"));
      }
      await upsertSection(manual.type, manual.styleName, manual.sectionKey, content);
