...
export default router.post(
  ...
  async (req, res) => {
    ...
    if (!catalogModels.find((m: any) => m.modelName === modelName)) {
      return res.status(400).send(error(`模型「${modelName}」尚未在渠道协议目录中定义，请联系管理员先添加该模型协议`));
    }

    const enabledModels = await u.vendor.getEnabledModelNames(id);
    if (enabledModels.includes(modelName)) {
      return res.status(200).send(success("该模型已启用"));
    }
    enabledModels.push(modelName);
    await u.vendor.setEnabledModelNames(id, enabledModels);
