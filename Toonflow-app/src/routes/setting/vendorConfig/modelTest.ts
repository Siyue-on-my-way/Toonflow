...
export default router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (type == "text") {
        const { textStream } = await u.Ai.Text(`${id}:${modelName}`, (req as any).user.id).stream({
        ...
        const reqFn = await u.Ai[aiTypeFn[type as "image" | "video"]](`${id}:${modelName}`, (req as any).user.id).run({
          ...reqConfig.modelData,
        });
