...
class Memory {
  ...
  constructor(agentType: string, isolationKey: string, userId: number, modelOverride?: string) {
    this.agentType = agentType;
    this.isolationKey = isolationKey;
    this.userId = userId;
    this.modelOverride = modelOverride as `${string}:${string}` | undefined;
  }
  ...
  private async generateSummary(contents: string[]): Promise<string> {
    ...
    const { text } = await u.Ai.Text(this.agentType as any, this.userId, undefined, 0, this.modelOverride).invoke({
