...
interface SkillPaths {
  // content 有值时（如从数据库读取的视觉/导演手册技法）直接使用，不再读取 path 对应的本地文件
  mainSkill: { path: string; name: string; description: string; content?: string }[];
  ...
  return {
    activate_skill: tool({
      ...
      execute: async ({ name }) => {
        ...
        let raw = "";
        try {
          raw = matched.content ?? (await fs.promises.readFile(matched.path, "utf-8"));
