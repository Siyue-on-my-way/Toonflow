...
async function getVendorTemplateFn(
  ...
  userId: number,
  taskHooks?: { onProviderTaskId?: (providerTaskId: string) => Promise<void> },
...
async function getVendorTemplateFn(fnName: FnName, modelName: `${string}:${string}`, userId: number, taskHooks?: { onProviderTaskId?: (providerTaskId: string) => Promise<void> }): Promise<any> {
  const [id, name] = modelName.split(/:(.+)/);
  if (!id || !name) throw new Error(`模型标识无效：${modelName}`);
  ...
  const expectedType = fnName === "textRequest" ? "text" : fnName === "imageRequest" ? "image" : fnName === "videoRequest" ? "video" : "tts";
  if (selectedModel.type !== expectedType) {
    throw new Error(`模型类型不匹配：${name} 不是${expectedType}模型`);
  }
  const vendorModule = u.vendor.getVendorModule(id);
  if (!vendorModule) throw new Error(`未找到供应商模块 id=${id}`);

  const vendorLabel = vendorModule.vendor?.name ?? id;
  // 渠道 Key / 已启用模型是全局配置（仅 admin 可写，参见 SIY-65），跟发起调用的是哪个账号无关
  if (!vendorConfigData.inputValues || vendorConfigData.inputValues === "{}") {
    throw new Error(`请先在设置页配置 ${vendorLabel} 渠道的 Key`);
  }
  const enabledModels = await u.vendor.getEnabledModelNames(id);
  if (!enabledModels.includes(name)) {
    throw new Error(`请先在设置页启用这个模型：${name}`);
  }

  const rawInputValues = JSON.parse(vendorConfigData.inputValues ?? "{}");
  ...
  const api = vendorModule.createVendorAPI(inputValues, {
    onProviderTaskId: taskHooks?.onProviderTaskId,
  });
  const fn = api[fnName];
...
class AiText {
  ...
  private modelOverride?: `${string}:${string}`;
  constructor(
  ...
  private async resolveModel(middleware?: any | any[]) {
    ...
    const modelName = this.modelOverride || (await resolveModelName(this.AiType));
    const sdkFn = await getVendorTemplateFn("textRequest", modelName, this.userId);
...
export type ReferenceList =
  | { type: "image"; base64: string; slot?: "start" | "end" }
  | { type: "audio"; base64: string; slot?: "start" | "end" }
  | { type: "video"; base64: string; slot?: "start" | "end" };
...
class AiImage {
  ...
  constructor(key: `${string}:${string}`, userId: number) {
    this.key = key;
    this.userId = userId;
  }
  async run(input: ImageConfig, taskRecord?: TaskRecord) {
    ...
    const exec = async (mn: `${string}:${string}`) => {
      const fn = await getVendorTemplateFn("imageRequest", mn, this.userId, {
        onProviderTaskId: taskRecord?.setProviderTaskId,
      });
...
class AiVideo {
  ...
  async run(input: VideoConfig, taskRecord?: TaskRecord) {
    ...
    try {
      const exec = async (mn: `${string}:${string}`) => {
        const fn = await getVendorTemplateFn("videoRequest", mn, this.userId);
        await referenceList2imageBase642(mn.split(/:(.+)/)[0], input);
...
class AiAudio {
  ...
  async run(input: VideoConfig, taskRecord?: TaskRecord) {
    ...
    const exec = async (mn: `${string}:${string}`) => {
      try {
        const fn = await getVendorTemplateFn("ttsRequest", mn, this.userId);
        await referenceList2imageBase642(mn.split(/:(.+)/)[0], input);
        this.result = await fn(input);
...
export default {
  Text: (
    ...
    thinkLevel?: 0 | 1 | 2 | 3,
    modelOverride?: `${string}:${string}`,
  ) => new AiText(AiType, userId, think, thinkLevel, modelOverride),
  Image: (key: `${string}:${string}`, userId: number) => new AiImage(key, userId),
  Video: (key: `${string}:${string}`, userId: number) => new AiVideo(key, userId),
  Audio: (key: `${string}:${string}`, userId: number) => new AiAudio(key, userId),
};
