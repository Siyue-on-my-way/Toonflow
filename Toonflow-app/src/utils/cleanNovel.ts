...
class CleanNovel {
  ...
  constructor(userId: number, concurrency: number = 5, textModel?: string) {
    this.emitter = new EventEmitter();
    this.concurrency = concurrency;
    this.userId = userId;
    this.textModel = textModel as `${string}:${string}` | undefined;
  }
  ...
  private async processChapter(novel: o_novel): Promise<EventType | null> {
    try {
      ...
      const resData = await u.Ai.Text("universalAi", this.userId, undefined, 0, this.textModel).invoke({
