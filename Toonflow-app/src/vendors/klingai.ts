...
export function createVendorAPI(inputValues: Record<string, string>) {
  ...
  const generateAuthToken = (): string => {
    ...
    const payload = {
      iss: inputValues.accessKey,
      exp: now + 1800,
      nbf: now - 5,
    };
    return jsonwebtoken.sign(payload, inputValues.secretKey, {
      algorithm: "HS256",
  ...
  const getBaseUrl = (): string => {
    return inputValues.baseUrl || "https://api-beijing.klingai.com";
  };
  ...
  const videoRequest = async (config: VideoConfig, model: VideoModel, inputValues: Record<string, string>): Promise<string> => {
    if (!inputValues.accessKey) throw new Error("缺少Access Key");
    if (!inputValues.secretKey) throw new Error("缺少Secret Key");
