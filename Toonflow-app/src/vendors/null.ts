...
export function createVendorAPI(inputValues: Record<string, string>) {
  ...
  const imageRequest = async (config: ImageConfig, model: ImageModel, inputValues: Record<string, string>): Promise<string> => {
    // The null vendor remains an empty development template for arbitrary
    // models. The named e2e fixture is intentionally deterministic so the
    // Quick Video integration tests can exercise MinIO writes and retries
    // without calling an external provider or consuming credits.
   ...
   *    如果确实需要可配置的常量值，必须将其声明在 inputValues 中，
   *    通过 inputValues.xxx 访问，让用户可在界面上配置。
