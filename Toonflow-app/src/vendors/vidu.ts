...
export function createVendorAPI(inputValues: Record<string, string>) {
  ...
  const imageRequest = async (imageConfig: ImageConfig, imageModel: ImageModel) => {
    ...
    const apiKey = inputValues.apiKey.replace("Token ", "");
    ...
    const createImageUrl = inputValues.baseUrl + "/reference2image";
  ...
  const checkTaskResult = async (taskId: string) => {
    const queryUrl = inputValues.baseUrl + "/tasks/{id}/creations";
  ...
  const videoRequest = async (videoConfig: VideoConfig, videoModel: VideoModel) => {
    ...
    const requestUrl = inputValues.baseUrl + "/start-end2video";
