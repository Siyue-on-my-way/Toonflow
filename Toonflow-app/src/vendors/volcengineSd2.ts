...
export function createVendorAPI(inputValues: Record<string, string>) {
  ...
  function deriveSigningKey(shortDate: string) {
    const kDate = crypto.createHmac("sha256", inputValues.sk).update(shortDate).digest();
  ...
  async function request(
    ...
    // 组装请求头
    const authorization = `HMAC-SHA256 Credential=${inputValues.ak}/${credentialScope}, SignedHeaders=${SIGNED_HEADERS}, Signature=${signature}`;
  ...
  function getTosRegion(): string {
    const ep = (inputValues.tosEndpoint || "").trim();
  ...
  function tosBucket(): string {
    return (inputValues.tosBucket || "").trim();
  }
  function tosEndpoint(): string {
    return (inputValues.tosEndpoint || "").trim();
  }
  function tosAk(): string {
    logger(inputValues.ak);
  
    return (inputValues.ak || "").trim();
  }
  function tosSk(): string {
    logger(inputValues.sk);
    return (inputValues.sk || "").trim();
  }
  function hasCompleteTosConfig(): boolean {
    return Boolean(tosEndpoint() && tosBucket() && tosAk() && tosSk());
  }
  function tosSecurityToken(): string {
    return (inputValues.securityToken || inputValues.sessionToken || "").trim();
  }
  ...
  const getHeaders = () => {
    ...
    return {
      ...
      Authorization: `Bearer ${inputValues.apiKey.replace(/^Bearer\s+/i, "")}`,
    };
  };
