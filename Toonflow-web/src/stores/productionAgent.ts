...
function makeProductionAgentStore(projectId: string) {
  return defineStore(`productionAgent-${projectId}`, () => {
    ...
    const textModel = ref(projectStore().project?.textModel || "");
    watch(
      () => projectStore().project?.textModel,
      (value) => {
        if (value !== undefined) textModel.value = value || "";
      },
    );
    const imageModel = ref(projectStore().project?.imageModel || "");
    watch(
      () => projectStore().project?.imageModel,
      (value) => {
        if (value !== undefined) imageModel.value = value || "";
      },
    );
    const sendChat = (content: string) => chat(content, undefined, textModel.value || undefined);
      ...
      (s) => {
        if (s) {
          ...
          s.on("generateStoryboard", async (data, callback) => {
            const storyData = await batchGenerateStoryboard(data.ids, false, data.imageModel);
    ...
    async function batchGenerateStoryboard(allIds: number[], compulsory: boolean = false, selectedImageModel?: string) {
      const effectiveImageModel = selectedImageModel || imageModel.value || undefined;
      try {
        const { data } = await axios.post("/production/storyboard/batchGenerateImage", {
          ...
          compulsory,
          ...(effectiveImageModel ? { imageModel: effectiveImageModel } : {}),
        });
        if (data) {
          const taskRecords = Array.isArray(data) ? data : [];
          taskRecords.forEach((record: any) => {
            const item = flowData.value.storyboard.find((storyboard) => storyboard.id === record.storyboardId);
            if (item) {
              item.state = record.status === "SUCCEEDED" ? "已完成" : record.status === "FAILED" ? "生成失败" : "生成中";
