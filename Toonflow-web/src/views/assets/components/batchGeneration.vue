  ...
  <div class="batchGeneration">
    <t-dialog
      ...
      <div class="content">
        ...
        <div class="f ac" style="margin: 10px 0; gap: 15px">
          <div class="f ac" style="width: 340px; gap: 8px">
            <span style="font-size: 14px; font-weight: 900; white-space: nowrap">{{ $t('workbench.assets.gen.selectModel') }}</span>
            <modelSelect v-model="batchModel" type="image" :changeConfig="true" @change="handleModelChange" />
          </div>
          <div class="f ac" style="width: 340px; gap: 8px">
            <span style="font-size: 14px; font-weight: 900; white-space: nowrap">{{ $t('components.modelSelect.type.text') }}</span>
            <modelSelect v-model="textModel" type="text" />
          </div>
          <imageGenParams
            v-model:ratio="batchAspectRatio"
            v-model:resolution="batchResolution"
            :model-detail="batchModelDetail"
            :default-ratio="project?.videoRatio ?? undefined"
            :default-resolution="project?.imageQuality ?? undefined"
            :show-upload="false"
            style="flex: 1" />
        </div>
        <t-table
...
import imageGenParams from "@/components/imageGenParams.vue";
...
// 批量生成参数（模型/比例/分辨率，选项随所选模型变化）
const batchModel = ref("");
const batchModelDetail = ref<any>(null);
const batchAspectRatio = ref("");
const batchResolution = ref("");
function handleModelChange(_val: string, data: any) {
  batchModelDetail.value = data ?? null;
}
// 生成图片
async function handleBatchGenerateImage() {
  ...
  if (!batchModel.value) {
    window.$message.warning($t('workbench.assets.gen.pickModel'));
    return;
  }
  if (!batchResolution.value || !batchAspectRatio.value) {
    window.$message.warning($t('workbench.assets.gen.pickResolution'));
    return;
  }
  // 检查是否所有选中的资产都有提示词
...
async function startGenerate(data: { id: number; prompt: string; name: string; type: string }) {
  ...
  try {
    const res = await axios.post("/assetsGenerate/generateAssets", {
      ...
      aspectRatio: batchAspectRatio.value,
    });
