<template>
  <div class="assets">
          ...
          <div class="panelContent">
            <div class="toolbar">
              <t-space>
                ...
                <t-button theme="primary" variant="outline" @click="handleUploadAttachment(item.value)">
                  <template #icon><t-icon name="upload" /></template>
                  {{ $t("workbench.assets.uploadAttachment") }}
                </t-button>
                <t-button theme="primary" variant="outline" @click="libraryPickerShow = true">
                  <template #icon><t-icon name="folder-open" /></template>
                  {{ $t("workbench.assetLibrary.import") }}
                </t-button>
    ...
    <t-dialog
      ...
      <div class="batch">
        ...
        <t-form labelAlign="top">
          ...
          <t-form-item :label="$t('components.modelSelect.type.text')" name="textModel" v-if="batchType === $t('workbench.assets.batchGenPrompt')">
            <modelSelect v-model="textModel" type="text" />
          </t-form-item>
    ...
    <library-picker v-if="project?.id" v-model="libraryPickerShow" :project-id="Number(project.id)" :type="assetOptions" @done="getFilteredData(assetOptions)" />
  </div>
</template>
...
const libraryPickerShow = ref(false);
// 新增
// 文件选择
const { open, onChange, onCancel } = useFileDialog({ multiple: true, reset: true, accept: ".png,.jpg,.jpeg,.webp,.mp3,.wav,.ogg,.mp4,.webm" });
const attachmentAcceptMap: Record<string, string> = {
  role: ".png,.jpg,.jpeg,.webp",
  tool: ".png,.jpg,.jpeg,.webp",
  scene: ".png,.jpg,.jpeg,.webp",
  clip: ".png,.jpg,.jpeg,.webp,.mp4,.webm",
  audio: ".mp3,.wav,.ogg,.m4a",
};
const MAX_ATTACHMENT_SIZE = 50 * 1024 * 1024;
const { open: openAttachment, onChange: onAttachmentChange, onCancel: onAttachmentCancel } = useFileDialog({ multiple: true, reset: true });
...
async function uploadProjectFile(file: File, type: string, retries = 2): Promise<void> {
  try {
    const base64Data = await readFileAsDataUrl(file);
    // type 恒传当前 Tab 的标准类型值，保证上传后能在当前 Tab 列表中看到
    await axios.post("/assets/uploadClip", { projectId: project.value?.id, base64Data, name: file.name, type });
  } catch (error) {
    if (retries > 0) return uploadProjectFile(file, type, retries - 1);
    throw error;
  }
}
async function uploadProjectFiles(files: FileList, type: string): Promise<void> {
  let failed = 0;
  for (const file of Array.from(files)) {
    try {
      await uploadProjectFile(file, type);
    } catch (error) {
      failed += 1;
      console.error(`上传 ${file.name} 失败`, error);
    }
  }
  if (failed) window.$message.warning(`${files.length - failed}/${files.length} ${$t("workbench.assets.uploadSuccess")}`);
  else window.$message.success($t("workbench.assets.uploadSuccess"));
  await getFilteredData(assetOptions.value);
}
async function handleUploadAttachment(type: string) {
  const files = await new Promise<FileList | null>((resolve) => {
    openAttachment({ accept: attachmentAcceptMap[type] ?? "" });
    onAttachmentChange((f) => resolve(f));
    onAttachmentCancel(() => resolve(null));
  });
  if (!files?.length) return;
  const valid = Array.from(files).filter((file) => file.size <= MAX_ATTACHMENT_SIZE);
  const oversized = files.length - valid.length;
  if (oversized) window.$message.warning(`${oversized} ${$t("workbench.assets.fileTooLarge")}`);
  if (valid.length) await uploadProjectFiles(valid as unknown as FileList, type);
}
async function handleAdd(type: string) {
  if (type === "clip") {
    ...
    if (files?.length) await uploadProjectFiles(files, type);
  } else if (type == "audio") {
...
watch(
  ...
  (value) => {
    if (value !== undefined) textModel.value = value || "";
  },
);
