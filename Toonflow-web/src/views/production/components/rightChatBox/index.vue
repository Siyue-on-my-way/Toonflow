      ...
      <t-chat-sender
        ...
        <template #footer-prefix>
          <div class="ac" style="gap: 5px">
            <div class="agentModelSelect">
              <span>{{ $t("components.modelSelect.type.text") }}</span>
              <modelSelect v-model="textModel" type="text" size="small" :disabled="status === 'pending' || status === 'streaming'" />
            </div>
            <div class="agentModelSelect">
              <span>{{ $t("components.modelSelect.type.image") }}</span>
              <modelSelect v-model="imageModel" type="image" size="small" :disabled="status === 'pending' || status === 'streaming'" />
            </div>
...
const { connected, messages, status, episodesId, loadingHistory, thinkLevel, textModel, imageModel } = storeToRefs(productionAgentStore());
...
const thinkLevelTheme = computed<"default" | "success" | "warning" | "danger">(() => ["default", "success", "warning", "danger"][thinkLevel.value] as "default" | "success" | "warning" | "danger" || "default");
