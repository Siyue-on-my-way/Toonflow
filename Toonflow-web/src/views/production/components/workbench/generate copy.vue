        ...
        <t-card :title="'#' + (activeTrackIndex + 1) + $t('workbench.generate.generateText')" header-bordered class="videoPrompt">
          <template #actions>
            <div class="promptActions f ac">
              <div class="textModelSelect f ac">
                <span>{{ $t("components.modelSelect.type.text") }}</span>
                <modelSelect v-model="textModel" type="text" size="small" :disabled="activeTrackGenTextLoading" />
              </div>
...
const modeList = computed(() => {
  ...
  const modes = (Array.isArray(modeOptions.value.mode) ? modeOptions.value.mode : []) as VideoModel["mode"];
  return modes.map((mode: VideoModel["mode"][number]) =>
        Array.isArray(mode)
...
const references = computed(() => {
  ...
  return uploadBox.value.filter((item) => item.src).map((item) => ({
