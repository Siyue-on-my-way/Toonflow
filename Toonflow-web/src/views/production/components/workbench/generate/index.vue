    ...
    <div class="generate ac">
      <div class="prompt" v-if="currentTrack">
        <t-card :title="'#' + (activeTrackIndex + 1) + $t('workbench.generate.generateText')" header-bordered class="videoPrompt">
          <template #actions>
            <div class="promptActions ac">
              <div class="textModelSelect ac">
                <modelSelect
                  v-model="textModel"
                  type="text"
                  size="small"
                  :disabled="currentTrack.state == '生成中'"
...
const textModel = ref(project.value?.textModel || "");
const textModelManuallySelected = ref(false);

interface AgentDeployItem {
  key?: string;
  modelName?: string;
}

interface AgentDeployResponse {
  qrdinaryData?: AgentDeployItem[];
  advancedData?: AgentDeployItem[];
}

async function loadDefaultTextModel() {
  if (textModel.value || textModelManuallySelected.value) return;

  try {
    const { data } = await axios.post<AgentDeployResponse>("/setting/agentDeploy/getAgentDeploy");
    const universalAi = [...(data?.advancedData ?? []), ...(data?.qrdinaryData ?? [])].find((item) => item.key === "universalAi");
    if (!textModelManuallySelected.value && !project.value?.textModel) {
      textModel.value = universalAi?.modelName || "";
    }
  } catch {
    // 保留空值，让模型选择器显示“请选择模型”，避免默认配置接口异常时阻塞分镜台。
  }
}

function handleTextModelChange(value: string) {
  textModelManuallySelected.value = Boolean(value);
}

watch(
  () => project.value?.textModel,
  (value) => {
    if (value && !textModelManuallySelected.value) textModel.value = value;
  },
);
...
function clampDuration(trackDuration: number): number {
  ...
  const durations = Array.isArray(drMap) ? drMap.flatMap((row) => row.duration ?? []) : [];
...
watch(
  ...
  (val) => {
    ...
    axios.post("/modelSelect/getModelDetail", { modelId: val }).then(({ data }) => {
      ...
      modelParmas.value.audio =
        data.audio === true || data.audio === "true" || data.audio === "optional" || data.audio === "always";
      const aspectRatioOptions = Array.isArray(data.aspectRatioOptions) ? data.aspectRatioOptions : [];
      if (aspectRatioOptions.length > 0 && !aspectRatioOptions.some((item: any) => item.value === modelParmas.value.aspectRatio)) {
        modelParmas.value.aspectRatio = aspectRatioOptions[0].value;
      } else if (aspectRatioOptions.length === 0 && !["16:9", "9:16"].includes(modelParmas.value.aspectRatio)) {
        modelParmas.value.aspectRatio = "16:9";
      }
      ...
      const modeMatched =
        currentParsed !== null &&
        (Array.isArray(data.mode) ? data.mode : []).some((m: VideoMode) => {
      ...
      if (!modeMatched) {
        const firstMode = Array.isArray(data.mode) ? data.mode[0] : undefined;
        if (firstMode) {
          const newMode = Array.isArray(firstMode) ? JSON.stringify(firstMode) : firstMode;
          modeChange(newMode);
        } else {
          modelParmas.value.mode = "";
        }
...
async function genText() {
  ...
  try {
    const { data } = await axios.post("/production/workbench/generateVideoPrompt", {
      ...
      textModel: textModel.value || undefined,
    });
    track.prompt = data;
    track.state = "已完成";
  } catch (e) {
...
onMounted(() => {
  ...
  modelParmas.value.aspectRatio = project.value?.videoRatio || "16:9";
  loadDefaultTextModel();
  getGenerateData();
...
const FRAME_REQUIRED_MESSAGES: Record<string, string> = {
  startEndRequired: "首尾帧模式需要同时提供首帧和尾帧",
  startFrameOptional: "首帧可选模式需要提供尾帧",
  endFrameOptional: "尾帧可选模式需要提供首帧",
};

/** 单个轨道生成视频 */
async function generateVideo() {
  const parsedMode = parseMode(modelParmas.value.mode);
  if (!parsedMode) {
    window.$message.error("请先选择视频模式");
    return;
  }
  const supportedMode = (modeOptions.value.mode ?? []).some((mode) => {
    if (Array.isArray(mode) && Array.isArray(parsedMode)) return JSON.stringify(mode) === JSON.stringify(parsedMode);
    return mode === parsedMode;
  });
  if (!supportedMode) {
    window.$message.error("当前视频模式不属于所选模型的协议能力，请重新选择");
    return;
  }
  if (parsedMode === "singleImage" && imageList.value.filter((item) => item.src).length !== 1) {
    window.$message.error("单图生视频模式需要且仅需要一张首图");
    return;
  }
  if (typeof parsedMode === "string" && FRAME_MODES.includes(parsedMode)) {
    const hasStart = Boolean(imageList.value[0]?.src && typeof imageList.value[0]?.id === "number");
    const hasEnd = Boolean(imageList.value[1]?.src && typeof imageList.value[1]?.id === "number");
    const missing =
      parsedMode === "startEndRequired"
        ? !hasStart || !hasEnd
        : parsedMode === "startFrameOptional"
          ? !hasEnd
          : !hasStart; // endFrameOptional
    if (missing) {
      window.$message.error(FRAME_REQUIRED_MESSAGES[parsedMode]);
      return;
    }
  }
  const currentAspectRatioOptions = modeOptions.value.aspectRatioOptions ?? [];
  if (currentAspectRatioOptions.length > 0 && !currentAspectRatioOptions.some((item) => item.value === modelParmas.value.aspectRatio)) {
    window.$message.error("请选择有效的视频比例");
    return;
  }
  const currentDurationOptions = (modeOptions.value.durationResolutionMap ?? []).flatMap((row) => row.duration ?? []);
  if (currentDurationOptions.length > 0 && !currentDurationOptions.includes(modelParmas.value.duration)) {
    window.$message.error("请选择有效的视频时长");
    return;
  }
  const promptLengthRange = modeOptions.value.promptLengthRange;
  const promptLength = currentTrack.value.prompt?.length ?? 0;
  if (promptLengthRange && promptLength > 0 && (promptLength < promptLengthRange[0] || promptLength > promptLengthRange[1])) {
    window.$message.error(`提示词长度需在 ${promptLengthRange[0]}-${promptLengthRange[1]} 个字符之间`);
    return;
  }
    ...
    onConfirm: async () => {
      ...
      try {
        const { data } = await axios.post("/production/workbench/generateVideo", {
            ...
            modelParmas.value.mode === "text"
              ...
              : (() => {
                  ...
                  const filtered = preSliced
                    .map((item, index) => {
                      if (!item.src || typeof item.id !== "number" || isNaN(item.id)) return null;
                      return {
                        id: item.id,
                        sources: item.sources,
                        ...(frameMode.includes(modelParmas.value.mode) && { slot: index === 0 ? "start" : "end" }),
                      };
                    })
                    .filter(Boolean);
          ...
          aspectRatio: modelParmas.value.aspectRatio,
          quality: modelParmas.value.quality,
