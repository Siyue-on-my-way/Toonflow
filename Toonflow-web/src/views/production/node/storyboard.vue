<template>
  <t-card class="storyboard">
    <div class="titleBar dragHandle pr">
      ...
      <t-button size="small" theme="primary" :loading="panelGenerateLoading" @click.stop="generateStoryboardPanel">
        {{ $t("workbench.production.node.storyboard.generatePanel") }}
      </t-button>
    ...
    <div class="content">
      ...
      <div class="ac" style="gap: 10px">
        ...
        <t-select
          ...
          :disabled="generateLoading || !imageModelOptions.length"
          placeholder="选择生图模型"
          style="min-width: 220px"
          @change="handleImageModelChange" />
        <t-button block @click="batchGenerateImage" :disabled="!storyboard.length || !selectedIds.length || !selectedImageModel || imageModelInvalid" :loading="generateLoading">
          {{ $t("workbench.production.node.storyboard.generateImage") }}
        </t-button>
      </div>
      <div v-if="imageModelInvalid" style="color: #e34d59; font-size: 12px; margin: 6px 0">
        项目默认图片模型不可用，请先在项目/供应商设置中修复或启用模型
      </div>
    </div>
...
const imageModelInvalid = ref(false);

async function loadImageModels() {
  imageModelsLoading.value = true;
  try {
    const { data } = await axios.post("/modelSelect/getModelList", { type: "image" });
    imageModelOptions.value = (data || []).map((model: any) => ({
      label: `${model.name} / ${model.label}`,
      value: `${model.id}:${model.value}`,
    }));
    const defaultModel = project.value?.imageModel || "";
    selectedImageModel.value = imageModelOptions.value.some((model) => model.value === defaultModel) ? defaultModel : "";
    imageModelInvalid.value = !selectedImageModel.value;
  } catch {
    imageModelOptions.value = [];
    selectedImageModel.value = "";
    imageModelInvalid.value = true;
  } finally {
    imageModelsLoading.value = false;
  }
}
function handleImageModelChange(value: unknown) {
  const normalized = typeof value === "string" ? value : String(value ?? "");
  selectedImageModel.value = normalized;
  imageModelInvalid.value = !normalized;
}
onMounted(loadImageModels);
watch(() => project.value?.imageModel, loadImageModels);
...
const panelGenerateLoading = ref(false);
async function generateStoryboardPanel() {
  if (storyboard.value.length) {
    const dialog = DialogPlugin.confirm({
      header: $t("workbench.production.node.storyboard.generatePanel"),
      body: $t("workbench.production.node.storyboard.generatePanelConfirm"),
...
async function createStoryboardPanel() {
  panelGenerateLoading.value = true;
  try {
    const store = productionAgentStore();
    const prompt = "请根据当前剧本、导演计划和分镜表，一键生成严格对应镜头1至镜头4的4条分镜面板数据。每条包含画面提示词、视频描述、时长和资产关联。只创建分镜条目，不生成图片或视频。";
    if (!store.chat(prompt)) throw new Error("Production agent is not connected");
    window.$message.success($t("workbench.production.node.storyboard.generatePanelStarted"));
  } catch (e) {
    window.$message.error($t("workbench.production.node.storyboard.generatePanelFailed"));
  } finally {
    panelGenerateLoading.value = false;
  }
}
async function batchGenerateImage() {
  ...
  try {
    await productionAgentStore().batchGenerateStoryboard(selectedIds.value, true, selectedImageModel.value);
