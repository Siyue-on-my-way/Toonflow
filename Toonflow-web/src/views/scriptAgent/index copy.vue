...
async function handleSelectAssets() {
  ...
  if (assets.length) {
    ...
    for (const a of assets.filter((asset) => asset.type !== "audio")) {
      if (!existing.has(a.id)) {
        scriptEditData.value.relatedAssets.push({ id: a.id, name: a.name, describe: a.describe, prompt: a.prompt, type: a.type as "role" | "scene" | "clip" | "tool" });
      }
    }
  }
}
